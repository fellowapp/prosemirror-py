...
if TYPE_CHECKING:
    ...
    from prosemirror.model.schema import Attrs, NodeType, Schema
...
class Node:
    ...
    def __init__(
        self,
        type: NodeType,
        attrs: Dict[str, Optional[Union[int, str]]],
    ...
    def mark(self, marks: List[Union[Any, Mark]]) -> "Node":
        if marks == self.marks:
            return self
...
class TextNode(Node):
    ...
    def mark(self, marks: List[Union[Any, Mark]]) -> "TextNode":
        return (
            self
            if marks == self.marks
...
def wrap_marks(marks: List[Union[Any, Mark]], str: str) -> str:
    i = len(marks) - 1
    while i >= 0:
