from typing import Literal, Callable, cast, List, Dict, Tuple, Any
...
from .schema import Schema, Attrs, NodeType, MarkType

from .to_dom import Element
...
class DOMParser:
	...
	def __init__(self, schema: Schema, rules: List[ParseRule]) -> None:
		...
		self.normalize_lists = not any([
			...
			for r in self._tags if r.node is not None and r.tag is not None and re.match(r'^(ul|ol)\b', r.tag) is not None
		])

	def parse(self, dom_: lxml.html.HtmlElement, options: ParseOptions | None = None) -> Node:
		if options is None:
	...
	def match_tag(self, dom_: DOMNode, context: "ParseContext", after: ParseRule | None = None) -> ParseRule | None:
		try:
		...
		for rule in self._tags[i:]:
			if (
				rule.tag and matches(dom_, rule.tag)
				and (rule.namespace is None or (dom_.prefix and dom_.nsmap[dom_.prefix] == rule.namespace))
	...
	def match_style(self, prop: str, value: str, context: "ParseContext", after: ParseRule | None = None) -> ParseRule | None:
		...
		for rule in self._styles[i:]:
			...
			if (
				...
				and (
					ord(style[len(prop)]) != 61
					or style[len(prop) + 1:] != value
				)
			):
				continue
...
def ws_options_for(_type: NodeType | None, preserve_whitespace: WSType, base: int) -> int:
	if preserve_whitespace is not None:
		return (OPT_PRESERVE_WS if preserve_whitespace else 0) | (OPT_PRESERVE_WS_FULL if preserve_whitespace == 'full' else 0)

	return OPT_PRESERVE_WS | OPT_PRESERVE_WS_FULL if _type is not None and _type.whitespace == 'pre' else base & ~OPT_OPEN_LEFT
...
class NodeContext:
	...
	def __init__(
		...
		if match is not None:
			...
			self.match = None if options & OPT_OPEN_LEFT or _type is None else _type.content_match
...
class ParseContext:
	...
	def __init__(self, parser: DOMParser, options: ParseOptions, is_open: bool) -> None:
		...
		top_options = ws_options_for(None, options.preserve_whitespace, 0) | (OPT_OPEN_LEFT if is_open else 0)

		if top_node:
			top_context: NodeContext = NodeContext(top_node.type, top_node.attrs, Mark.none, Mark.none, True,
													options.top_match or top_node.type.content_match, top_options)
		elif is_open:
			top_context = NodeContext(None, None, Mark.none, Mark.none, True, None, top_options)
		else:
			top_context = NodeContext(parser.schema.top_node_type, None, Mark.none, Mark.none, True, None, top_options)
	...
	def add_text_node(self, dom_: DOMNode) -> None:
		...
		if (
			...
			if not (top.options & OPT_PRESERVE_WS):
				...
				if re.search(r'^[ \t\r\n\u000c]', value) is not None and self.open == len(self.nodes) - 1:
					node_before = top.content[-1]
					dom_node_before = dom_.getprevious()
					if (
						node_before is None
						or (dom_node_before is not None and dom_node_before.tag.upper() == 'BR')
						or (node_before.is_text and re.search(r'[ \t\r\n\u000c]$', cast(TextNode, node_before).text) is not None)
					):
						value = value[1:]
		...
		for i, style in enumerate(styles):
			...
			while i < len(styles) - 1:
				...
				if rule.clear_mark is not None:
					...
					add = self.parser.schema.marks[rule.mark].create(rule.attrs).add_to_set(add)
	...
	def add_element_by_rule(self, dom_: DOMNode, rule: ParseRule, continue_after: ParseRule | None = None) -> None:
		...
		if node_type and node_type.is_leaf:
			...
			self.find_inside(dom_)
			rule.get_content(dom_, self.parser.schema).for_each(lambda node: self.insert_node(node))
		else:
			content_dom = dom_
	...
	def enter_inner(self, type_: NodeType, attrs: Attrs | None = None, solid: bool = False, preserve_ws: WSType = None) -> None:
		...
		self.nodes.append(
			NodeContext(type_, attrs, top.active_marks, top.pending_marks, solid, None, options)
		)
	...
	def matches_context(self, context: str) -> bool:
		...
		use_root = not self.is_open and (option is None or option.parent.type == self.nodes[0].type)
	...
	def textblock_from_context(self) -> NodeType | None:
		...
		if context:
			d = context.depth
			while d >= 0:
				default = context.node(d).content_match_at(context.index_after(d)).default_type

				if default is not None and default.is_textblock and default.default_attrs:
					return default
	...
	def remove_pending_mark(self, mark: Mark, upto: NodeContext) -> None:
		...
		while depth >= 0:
			...
			try:
				found = level.pending_marks.index(mark)
			except ValueError:
				level.active_marks = mark.remove_from_set(level.active_marks)
				stash_mark = level.pop_from_stash_mark(mark)

				if stash_mark is not None and level.type is not None and level.type.allows_mark_type(stash_mark.type):
...
def compare_document_position(node1: DOMNode, node2: DOMNode) -> int:
	if not isinstance(node1, lxml.etree._Element) or not isinstance(node2, lxml.etree._Element):
		raise ValueError("Both arguments must be lxml HtmlElement objects.")
...
def get_node_type(element: DOMNode) -> int:
	# Check if the element is an lxml HtmlElement
